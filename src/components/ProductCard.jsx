...
const ProductCard = React.memo(function ProductCart({ product = {}, isDisplayGrid = true })  {
    ...
    const handleAddToCart = useCallback((productId, colorId, quantity) => {
        toast.promise(
            apiRequest.post(
                '/cart',
    ...
    useEffect(() => {
      if (isVisible && product?.colors?.length && selectedColor) {
        const img1 = new Image();
        img1.src = selectedColor.images[0];
        const img2 = new Image();
        img2.src = selectedColor.images[1];
      }
    }, [isVisible, product, selectedColor]);

    const productImage = useMemo(() => {
        return product?.colors?.length && selectedColor && selectedColor?.images[0];
    }, [product, selectedColor]);

    const productHoverImage = useMemo(() => {
        return product?.colors?.length && selectedColor && selectedColor?.images[1];
    }, [product, selectedColor]);

    console.log("ProductCard rendering, isVisible:", isVisible);
