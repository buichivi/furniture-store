            ...
            products.forEach((prod) => {
                ...
                if (categoryMap.has(category.name)) {
                    categoryMap.set(category.name, {
                        category,
                        numberOfProducts:
                            categoryMap.get(category.name).numberOfProducts + 1,
                    });
                }
            });
        }
        // eslint-disable-next-line no-unused-vars
        return Array.from(categoryMap).filter(
            ([_, item]) => item.numberOfProducts > 0,
        );
    }, [categories, products]);
    ...
    return (
        <div ref={sliderCategoryRef}>
            {!isVisible ? (
                ...
                <Swiper
                    slidesPerView={
                        listCategory.length < 4 ? listCategory.length : 4
                    }
                ...
                >
                    {/* eslint-disable-next-line no-unused-vars */}
                    {listCategory.map(
                        ([_, { category, numberOfProducts }], index) => {
                            return (
